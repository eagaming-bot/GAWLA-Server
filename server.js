...
const SPY_BANKS = {
  job: {
    label: "وظيفة",
    items: ["طبيب", "مدرس", "مهندس", "طيار", "ممرضة", "شيف", "محامي", "شرطي", "مصور", "نجار", "سائق تاكسي", "صيدلي", "مذيع", "كهربائي", "بائع", "مبرمج", "محاسب", "مزارع", "صياد", "خباز", "جزار", "حلاق", "خياط", "ميكانيكي", "سباك", "نادل", "بواب", "حارس أمن", "مترجم", "صحفي", "مهندس معماري", "طبيب أسنان", "بيطري", "مدرب رياضي", "لاعب كورة", "ممثل", "مغني", "راقصة", "مصمم أزياء", "رسام", "كاتب", "محرر", "مذيع رياضي", "طيار حربي", "ضابط جيش", "رجل إطفاء", "سائق أتوبيس", "سائق ميكروباص", "عامل بناء", "عامل نظافة", "موظف بنك", "صراف", "مدير فندق", "دليل سياحي", "مضيفة طيران", "كابتن سفينة", "صياد أسماك", "نجار موبيليا", "فني تكييف", "فني موبايلات", "مصمم جرافيك", "مبرمج ألعاب", "محقق بوليسي", "قاضي", "وكيل نيابة", "إمام مسجد", "شيخ", "قسيس", "عالم فضاء", "عالم كيمياء"],
  },
  place: {
    label: "مكان",
    items: ["المطار", "الشاطئ", "المستشفى", "المدرسة", "السينما", "المطعم", "الجيم", "السوق", "المسجد", "الحديقة", "المكتبة", "الفندق", "المصنع", "المحطة", "الاستاد", "الصيدلية", "الجامعة", "الحضانة", "النادي", "المول", "البنك", "المكتب", "الكافيه", "الصالون", "المخبز", "الجزارة", "محطة البنزين", "الورشة", "المتحف", "المسرح", "دار السينما", "حديقة الحيوان", "الأكواريوم", "المرسى", "الميناء", "محطة القطر", "موقف الأتوبيس", "السفارة", "المحكمة", "القسم", "الاستراحة", "الشاليه", "الفيلا", "العمارة", "المصيف", "الملاهي", "السيرك", "البازار", "السوق الشعبي", "المعرض", "قاعة الأفراح", "صالة الرياضة", "حمام السباحة", "الكورنيش", "السرايا", "المزرعة", "الاسطبل", "بيت الطيور", "المحجر", "المنجم"],
  },
  food: {
    label: "أكلة",
...
const io = new Server(server, {
  cors: {
    origin: "*", // بيسمح لأي دومين إنه يتصل - مفيد لو نسخة الويب اتستضافت
    // على دومين منفصل (زي Vercel) بدل ما تبقى على نفس دومين السيرفر
    methods: ["GET", "POST"],
  },
});
...
function newRoom(hostSocketId) {
  return {
    ...
    // تتبّع كام مرة متتالية كل لاعب طلع جاسوس - بيفضل موجود عبر كل الماتشات
    // فى نفس الغرفة (مش بيتصفر مع room.spy)، عشان نمنع نفس الشخص من التكرار
    // أكتر من مرتين ورا بعض
    spySpyStreaks: {},
...
// بيختار جاسوس/جواسيس من غير ما يسمح لنفس اللاعب يتكرر أكتر من مرتين
// متتاليين - لو حد وصل للحد، بيتستبعد من الاختيار (إلا لو مفيش مرشحين
// كفاية غيره، وقتها بيرجع مسموح كحل أخير)
function pickSpiesAvoidingStreak(playerIds, spyCount, streakMap) {
  const eligible = playerIds.filter((id) => (streakMap[id] || 0) < 2);
  const pool = eligible.length >= spyCount ? eligible : playerIds;
  const shuffled = strongShuffleArray(pool);
  const selected = shuffled.slice(0, spyCount);
  const selectedSet = new Set(selected);
  const newStreak = {};
  for (const id of playerIds) {
    newStreak[id] = selectedSet.has(id) ? (streakMap[id] || 0) + 1 : 0;
  }
  return { selected, newStreak };
}
...
function startSpyMatch(roomCode, category) {
  ...
  if (!room.spySpyStreaks) room.spySpyStreaks = {};
  const { selected, newStreak } = pickSpiesAvoidingStreak(playerIds, 1, room.spySpyStreaks);
  room.spySpyStreaks = newStreak;
  const spyId = selected[0];
...
// التنقل الداخلي بين الشاشات، فمش محتاجين مسارات منفصلة على السيرفر.
// لازم يكون آخر route قبل ما السيرفر يشتغل، عشان أي API/سوكيت يتاخد الأولوية.
app.get(/.*/, (req, res) => {
